...
def ExtracttoDB():
    start = time.time()
    FILE= fw.FILE_NAME_PATTERN(_P.DATA_PATH,starts_with="nasdaq_stocks_")
    data = json.loads(fw.FILE_READ(FILE,_P.DATA_PATH))
    for c in data:
        text  = "INSERT INTO stocks_NASDAQ (symbol,name,lastsale,netchange,pctchange,volume,marketCap,country,ipoyear,industry,sector,url) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)"
